...
export default function VipAdminCategories() {
  ...
  const fetchCategories = async () => {
    try {
      const snapshot = await getDocs(collection(db, 'vip_categories'));
      ...
      handleFirestoreError(error, OperationType.GET, 'vip_categories');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddCategory = async (e: FormEvent) => {
    e.preventDefault();
    try {
      await addDoc(collection(db, 'vip_categories'), {
      ...
      handleFirestoreError(error, OperationType.CREATE, 'vip_categories');
    }
  };
  ...
  const handleDelete = async () => {
    if (deletingId) {
      try {
        await deleteDoc(doc(db, 'vip_categories', deletingId));
        fetchCategories();
      } catch (error) {
        handleFirestoreError(error, OperationType.DELETE, `vip_categories/${deletingId}`);
      } finally {
        setDeletingId(null);
      }
    }
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[50vh]">Loading VIP categories...</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <h1 className="text-2xl font-bold text-gray-900">VIP Categories Management</h1>
      ...
      {isAdding && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
          <h2 className="text-xl font-bold mb-4 text-purple-900">Add New VIP Category</h2>
          <form onSubmit={handleAddCategory} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
              <input required type="text" value={newCategory.name} onChange={e => setNewCategory({...newCategory, name: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500" />
            </div>
            <div>
            ...
            <div className="flex justify-end space-x-4 pt-4">
              ...
              <button type="submit" className="px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700">Save VIP Category</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[600px]">
          ...
          <tbody>
            ...
            {categories.length === 0 && (
              <tr>
                <td colSpan={2} className="p-8 text-center text-gray-500">No VIP categories found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <ConfirmModal
        isOpen={!!deletingId}
        title="Delete VIP Category"
        message="Are you sure you want to delete this VIP category? This action cannot be undone."
