...
export default function AdminBanners() {
  ...
  const handleDelete = async () => {
    if (deletingId) {
      try {
        await deleteDoc(doc(db, 'banners', deletingId));
        fetchBanners();
      } catch (error) {
        handleFirestoreError(error, OperationType.DELETE, `banners/${deletingId}`);
      } finally {
        setDeletingId(null);
      }
    }
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
          ...
          <form onSubmit={handleAddBanner} className="space-y-4">
            <div>
              ...
              <input required type="text" value={newBanner.title} onChange={e => setNewBanner({...newBanner, title: e.target.value})} className="w-full p-2 border border-gray-300 rounded-md" />
            </div>
            <div>
            ...
            <div className="flex items-center">
              <input type="checkbox" id="isActive" checked={newBanner.isActive} onChange={e => setNewBanner({...newBanner, isActive: e.target.checked})} className="mr-2" />
            ...
            <div className="flex justify-end space-x-4 pt-4">
              ...
              <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Save Banner</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[600px]">
          ...
          <tbody>
            ...
            {banners.length === 0 && (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-500">No banners found.</td>
              </tr>
            )}
          </tbody>
        </table>
